...
public class DateUtils {
    ...
    public static Date getSpecifiedMonthFromNow(byte type, int month){
        ...
        switch (type){
            case TYPE_AFTER:
                localDate.plus(month, ChronoUnit.MONTHS);
                break;
            case TYPE_BEFORE:
                localDate.minus(month, ChronoUnit.MONTHS);
                break;
            default:
        }
        return localDateToDate(localDate);
    ...
    public static Date getSpecifiedYearFromNow(byte type, int year){
        ...
        switch (type){
            case TYPE_AFTER:
                localDate.plus(year, ChronoUnit.YEARS);
                break;
            case TYPE_BEFORE:
                localDate.minus(year, ChronoUnit.YEARS);
                break;
                default:
        }
        return localDateToDate(localDate);
    ...
    public static void main(String[] args) {
        Date d = getSpecifiedDayFromNow(TYPE_AFTER, 2);
